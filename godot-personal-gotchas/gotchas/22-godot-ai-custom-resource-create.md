...
**Version boundary (POST-#583 — now merged):** the above is the PRE-#583 / pre-2.8.1 behavior — the create path is ClassDB-only, so a custom `class_name` Resource is entirely unreachable (`Unknown resource type`). **PR #583 merged upstream 2026-06-30, shipped in godot-ai 2.8.1**: the create path resolves the project class via `ProjectSettings.get_global_class_list()` and instead gates on `Script.can_instantiate()` — a NON-`@tool` script then fails `WRONG_TYPE … add @tool` rather than `Unknown resource type`. So on **2.8.1+** custom `class_name` Resources ARE reachable (the hand-write workaround above is only needed pre-2.8.1). See **#40** for the successor entry (the `@tool` gate + fix).
