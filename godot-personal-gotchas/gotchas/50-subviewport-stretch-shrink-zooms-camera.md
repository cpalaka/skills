...
**Symptom** — You wrap a 2D scene in a `SubViewportContainer` (`stretch=true`) and raise
...
**Cause** — `stretch_shrink = N` divides the SubViewport's pixel dimensions by N (a 1920×1080
...
**Fix** — Compensate the camera zoom by the same factor: `camera.zoom = base_zoom / stretch_shrink`
...
**Detect proactively** — Any `SubViewportContainer` with `stretch_shrink > 1` wrapping a `Camera2D`
...
**Confirmed by** — space-miner-prototype pixel-filter slice (TASK-002), 2026-06-29. A whole-world
