...
**Symptom**
On godot-ai **2.8.1+** (PR #583, merged upstream 2026-06-30), `resource_manage op=create type="MyThing"` — or a nested `{"__class__": "MyThing", ...}` value — on a project script `class_name MyThing extends Resource` returns:
...
**Cause**
The create path (`resource_handler.gd` — the `ProjectSettings.get_global_class_list()` loop at `:232`) resolves the project class, gates on a native-base-type check (`get_instance_base_type()` + `is_parent_class(.., "Resource")` at `:246`–`247`) and then on `Script.can_instantiate()` (`:248`), returning the `WRONG_TYPE … add @tool` message at `:249`. Godot returns `can_instantiate() == false` for a NON-`@tool` script when called inside the editor process — the editor only instantiates `@tool` scripts. Correct Godot behavior, NOT a godot-ai defect; the `WRONG_TYPE` message is PR #583's honest error-split — distinct from `INTERNAL_ERROR` (script-load failure, `:230`/`:265`), the required-arg-`_init()` reject (`:261`), and `VALUE_OUT_OF_RANGE`/"Unknown resource type" (`:267`, fires ONLY when the class is genuinely unregistered). Same `can_instantiate()` semantics as #28 (there it reports compiled-validity, here editor-time-instantiability, which excludes non-`@tool` scripts). (Cites verified against the merged godot-ai 2.8.1 source.)
...
**Confirmed by**
Live dogfood verification 2026-06-20 (then on the PR #583 branch `feat/instantiate-custom-resources` @ `61b72bf`); **PR #583 merged upstream 2026-06-30, shipped in godot-ai 2.8.1** — now released behavior, not a patched-branch quirk. Handler source (merged 2.8.1): `addons/godot_ai/handlers/resource_handler.gd` create path ~`:232`–`267`. See project `docs/godot-gotchas.md`.
