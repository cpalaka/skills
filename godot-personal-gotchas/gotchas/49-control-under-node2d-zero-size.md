...
A full-rect Control (the "Full Rect" anchors preset → `anchor_right=1`, `anchor_bottom=1`,
...
A Control resolves its anchors against its parent's area (`get_parent_area_size()`). That
...
Parent the Control under a **CanvasLayer** (or another Control, or make it the scene root). A
...
Grep `.tscn` for a `SubViewportContainer` (or any full-rect Control you expect
...
space-miner-prototype pixel-filter slice (TASK-002), 2026-06-29. A `SubViewportContainer`
