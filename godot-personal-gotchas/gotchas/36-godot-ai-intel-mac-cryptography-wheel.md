...
**Confirmed by**
2026-06-17 (space-miner-prototype) — diagnosed and verified end-to-end; server boots and binds `127.0.0.1:8000` + `127.0.0.1:9500`. This is an environment (per-machine, Intel mac) gotcha, not a code one — unlike the godot-ai writer/RPC-surface entries (#19, #22–#25). Sibling to #37 (the Apple Silicon arm64 variant — same symptom + dependency chain, different root cause and fix). Cross-references `docs/godot-mcp-guide.md`.
