...
|---|---|---|
| **Inferable** — the error names its own fix | 2, 12, 17, 20, 40, 87 | The compiler already says it. All six **keep their mechanized scan check** — prevention is free, and a check does not need an index row. #17 (forward is −Z) is documented engine convention; #40's error literally reads "add `@tool` to instantiate it here". |
