...
`godot-ai` (`hi-godot/godot-ai`, MIT) is the **primary writer** in the recommended setup — scene/node/script/property creation, `input_map_manage`, `script_patch`, `project_run`, `editor_screenshot`, `logs_read`. It writes most struct types correctly — which is why it's the writer over godot-mcp — but it is **not** universal: it silently no-ops `Vector2i`/`Vector3i`, serializes typed resource collections as bare path strings, can't create custom `class_name` Resources, and drops signal connections on `scene_save` (see `godot-gotchas.md`). godot-mcp stays as the read/test complement. Skip this only if the project will write through godot-mcp (not recommended — godot-mcp silently no-ops `Rect2`).

Prerequisite: `uv` on PATH (the godot-ai dock auto-starts a uv-managed Python server on `:8000` + `:9500`).

1. **Vendor the addon (pinned).** From a `git clone` of `hi-godot/godot-ai`, **check out the `v2.7.5` release tag** (`git checkout v2.7.5`) before copying — then copy the install-ready `addons/godot_ai/` into the project's `addons/`. (The copy with `plugin.cfg` is typically nested at `plugin/addons/godot_ai/`, not the repo root; a `src/godot_ai` copy may also exist and is NOT the one to vendor. Defer to the current README if the layout differs.) The HTTP URL is not version-pinned, so **the checked-out tag is the pin** — pinning a tag instead of cloning `main` is what stops the cross-project version drift. Validated against **v2.7.5** (v2.7.x added `game_manage`: named-action input injection + numerical runtime-state read — the headless self-test the stack otherwise needs godot-mcp for).
