...
- **Branch first per task** — before the first commit of backlog-task work, create a feature branch (`task-NNN-<slug>`); merge to `main` with `--no-ff` after sign-off sets Done (one merge commit per task). Direct-to-main remains fine for non-task chores (board/docs housekeeping).
...
- **Milestones = phases**; **labels = free-form** (multiple per task/draft via `-l a,b`), added organically as themes emerge. **Provenance: every task/draft created through Claude carries `claude-generated`**, set in `-l` at create time (drafts have no CLI edit verb — it can't be retrofitted later); items the user enters directly (web UI) stay unlabeled.
