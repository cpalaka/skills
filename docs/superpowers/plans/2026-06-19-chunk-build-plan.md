...
- [x] 6. Migrate a godot project — **DONE 2026-06-19** (space-miner-prototype: CLAUDE.md → 3-zone `dev-base + git-flow-squash`, no backlog-core/no board, zero-loss adversarial behavior diff, merged + pushed; godot-profile audit fixed at SOURCE — 12 findings incl. a live `game_manage` confirmation on 2.7.5/4.7 and the engine conditional-knob fix; project `docs/godot-mcp-guide.md` re-synced)
- [x] 7. Retire `init-backlog-project` + `init-godot-claude-project` — DONE 2026-06-19. Both deleted (replaced by `init-project` + profiles); `~/.claude/skills` symlinks repointed to `init-project`; live references de-pointed in `sync-godot-skills`, `README`, `CONTEXT.md`, and the godot templates. Historical design docs/ADRs left intact.
