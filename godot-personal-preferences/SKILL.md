...
| 6 | About to claim "all scripts compile" or "compile clean" from a green GUT run | Don't. GUT only compiles scripts its tests/targets touch. Run `--headless --check-only --quit` for parse coverage (when editor closed), OR `mcp__godot-mcp__godot_editor get_log_messages source="editor"` (when editor open), OR godot-ai `logs_read` after a `project_run`. **But `--check-only` is NOT exhaustive either** — it misses runtime-compile errors (e.g. array-literal-index `:=` → Variant, gotcha #44) that fire only at real load. The real backstop: make the suite actually COMPILE every script — a behavioral test for logic, a **preload-smoke test** (`const X := preload(...)` + `X.new()`/free, or `load(scene).instantiate()`) for controllers/support scripts with no other coverage. When capture is down (gotcha #43) you can't run it — treat the script as UNVERIFIED, never substitute static checks + `is_playing:true` |
