...
| 3 | _(retired 2026-07-25)_ explicit `godot-personal-gotchas` invocation, auto-activation, skill loader | Retired — the loader fires reliably now; don't reinstate. Body kept: `preferences/3-*.md` |
...
| 5 | Before instructing on Godot 4.x class API specifics, Inspector workflow, or sub-resource property names | Fetch docs before composing. Two sources, and the split matters (corrected 2026-08-07): godot-ai `api_manage(op="get_class")` returns version-correct ClassDB **metadata** live from the connected editor — property/method/signal/enum/constant names and signatures, which is what "sub-resource property names" actually needs; `mcp__godot-mcp__godot_docs fetch_class <ClassName>` returns the class-reference **prose** (`section: "description"`), which ClassDB does not carry. godot-ai is **not** docs-blind, so that alone is no longer a reason to keep godot-mcp connected — keep it for the prose and for `godot_runtime_state`'s hz-sampled watch. (Caller-facing op is `get_class`; the GDScript handler is `get_class_info` — the latter is rejected.) Training-data drift to older 4.x / 3.x conventions is the failure mode |
| 6 | About to claim "all scripts compile" or "compile clean" from a green GUT run | Don't. GUT only compiles scripts its tests touch. Parse coverage: `--headless --check-only --quit` (editor closed) or, with the editor open, godot-ai `logs_read source="editor"` — **not** godot-mcp's `get_log_messages source="editor"`, whose `source` arg is a phantom that has never existed and is silently stripped (measured 2026-08-07; see gotcha #80's sibling note in the tooling section of `godot-personal-gotchas`). Neither catches runtime-compile errors (**#44** — array-literal `Variant` inference, "blind to BOTH `--check-only` and the LSP"; see also **#96**, "preload is not a parse gate"). The real backstop is a **preload-smoke test**. Capture down (#43) → treat the script as UNVERIFIED |
