...
The source of truth for personally hand-authored Claude Code skills, plus the machinery that keeps
them in step with the projects that use them (the `init-project` engine + Profiles, the `chunks/`
library, `audit-godot-parity`, `skill-updater`).

## Load-bearing facts (get these wrong and you break live skills)

- **Editing a file here is LIVE.** `~/.claude/skills/<name>` symlinks *into* this repo, so a change
  to `<skill>/SKILL.md` changes the running skill immediately — there is no install step. Verify a
  change by invoking the affected skill, not just by re-reading the file.
...
- **Chunk vs Template** (full detail in CONTEXT.md): a **Chunk** (`chunks/`) is single-source and
  ...
  **Template** (`init-project/profiles/<type>/templates/`) is *copied* into a project at init and
  thereafter kept aligned via a parity check. Know which you're editing.
