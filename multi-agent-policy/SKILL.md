...
**`insight` is measured and unsupported — don't reach past `critic` without a reason this run is different.** A/B on one lens and one diff, N=3 per tier: scarce found *fewer* findings (5.67 vs 6.67) and zero HIGHs against workhorse's one, and across-tier disagreement (Jaccard 0.51) never cleared the within-tier noise floor (0.61) — two runs of the same tier differ as much as the tiers do. The only clean asymmetry favoured workhorse 3/3 vs 0/3. Caveat honestly: one lens, one diff — and the tier-unique findings split by *kind* (scarce caught a shader and a doc, workhorse caught shell control flow), so a shader- or spec-heavy subject may invert it. Full method and scope limits: ADR 0006 in `skills`. Where the two tiers genuinely differ is *what* they look at, and vendor lenses buy that diversity more cheaply than 2× token price.
...
- **Injecting mid-run context: edit the SCRIPT for a resume, but reach live agents through the artifacts they were told to read.** *Claude Code:* a running workflow uses the script as loaded at launch, so edits take effect only on `resumeFromRunId`. Findings measured after dispatch reach in-flight agents only if the prompt already pointed them at a file you can append to (a board row, a plan doc) — which is a reason to point every agent at the task row rather than inlining the whole brief. Append only to the stages you intend to re-run, or the resume invalidates the cache for everything downstream of the edit. *Codex:* use the active task/subagent messaging surface rather than assuming script-resume semantics — but the point-agents-at-an-appendable-artifact habit is host-independent and buys the same reach.
- **Heartbeat**: any background delegate expected to exceed ~10 minutes gets a calibrated liveness check. Undelegated solo work and interactive work need no heartbeat. On either host, no growth AND no commit → one liveness probe (`find <scope> -mmin -12`) before declaring a hang — a static tree is also what a gate-run looks like.
