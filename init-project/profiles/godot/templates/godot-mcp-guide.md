...
- **No mouse / cursor-coordinate support in `godot_input`.** It cannot move the pointer or click a screen position. godot-ai's `game_manage input_mouse` *does* inject raw mouse button/motion events (for handlers reading `event.position`), **but** a handler reading `get_global_mouse_position()` still won't see them — that reads the OS cursor, not the injected event (`docs/godot-gotchas.md` #32). So for cursor-**aim** mechanics (an aim indicator, a node reading `get_global_mouse_position()`) there's still no clean injection route: move the real OS mouse and verify with a `source="game"` screenshot, hold the action stationary, or temporarily drive the value from a script. (`godot_runtime_state` is likewise `action`-discriminated: `digest` | `watch_start` | `watch_collect` | `watch_stop` — not `operation`/`get_property`.)
