...
- **godot-ai** (`hi-godot/godot-ai` v3.1.3, vendored at `addons/godot_ai/`; added 2026-05-30, last bumped 2026-08-07) — construction-grade editor MCP over HTTP `http://127.0.0.1:8000/mcp` (its Python server is also a WS *server* on :9500; the editor plugin dials out). 41 tools / 135 ops: `scene_*`/`node_*`/`script_*`, `signal_manage`, `material`/`particle`/`camera`/`animation`/`ui_manage`, `project_run`, `editor_screenshot`, `logs_read`, `game_manage` (v2.7.x: **raw** key/mouse/gamepad input injection — `input_key`/`input_mouse`/`input_gamepad`, which drive any *mapped* action — plus runtime node-state read `get_node_info`/`get_scene_tree` and named-action-state read `input_state`; verified live on 2.7.5 / Godot 4.7). Enabled as a project plugin; the dock auto-starts the uv server. Upgrades *construction* over godot-mcp's run/edit baseline — but the two heavily overlap (scene/node/script), so **prefer one writer per editor instance** across them.
...
| **Inject input by timed NAMED action** (action_name + precise start/hold durations) | **godot-mcp** `godot_input` | godot-ai's `game_manage` *also* injects input, but only RAW key/mouse/gamepad single events — godot-mcp injects by Input-Map **action** with sequenced start/duration timing (the headless self-test of input mechanics). |
| **Numerical runtime state** — hz-sampled time-series (pos/vel/rot over time + event detection) | **godot-mcp** `godot_runtime_state` | godot-ai's `game_manage get_node_info` reads a runtime *snapshot* too; godot-mcp's edge is the digest/**watch** hz-sampled time-series that verifies physics MATH a single snapshot can't. |
| **Version-correct class docs** | **godot-mcp** `godot_docs` | EXCLUSIVE — only godot-mcp has it. |
| **Parse errors / scene won't load** | **godot-ai** `logs_read source="editor"` | `get_errors` + `get_debug_output` do NOT exist in v3.6.1 — this replaces them. |
...
- **godot-ai `editor_screenshot` arg is `source=`, not `target=`** — `target=...` is a hard pydantic validation error (`Unexpected keyword argument`). And the default `source="viewport"` captures the *editor 3D viewport*, which errors `EDITOR_NOT_READY (viewport_not_3d)` on a 2D scene (no Node3D in the tree). To capture a **running 2D game**, pass `source="game"` (add `max_resolution=0` for native res); the image returns inline in the tool result — no file is written to the `screenshots/` dir, so don't `Read` a path afterward. For a 3D scene with no live game, use `source="cinematic"` (renders through a `Camera3D`). Valid sources: `viewport` | `cinematic` | `game`.
...
godot-ai is multi-editor/multi-session **by design** (verified in the v3.1.3 source): the Python server keeps a per-session registry (`server.py` adoption guard + `orphan_reaper.py`), each editor plugin registers a session id `<dir-slug>@<4hex>` (`connection.gd:_make_session_id` → `"%s@%s" % [slug, _rand_hex(4)]`) (the hex suffix exists to disambiguate same-project twins), and a second editor instance finding a compatible running server *adopts* it rather than fighting for the port. "One writer per editor instance" is the real constraint — commands run serially on each editor's main thread; a second editor on a worktree is a second independent writer.

Rules when running two editor-writing agents:

- **Exact `session_id` on every call.** Every godot-ai tool accepts an optional `session_id` (empty = the server-global active session). Each agent runs `session_manage op=list` once at start, copies its exact session id, and passes it explicitly on EVERY godot-ai call. Ids must match exactly — substring matching exists only in `session_activate`.
- **Never `session_activate` in multi-editor mode.** The active-session default is a single server-global pointer; two agents calling `session_activate` clobber each other's routing. Explicit per-call `session_id` bypasses the pointer entirely.
- **Distinct worktree directory basename.** The session slug derives from the project directory name. And never open the *same checkout* in two editors — they share `.godot/` and clobber each other's caches; worktrees get their own `.godot/`.
- **Fresh worktree: import first.** A new worktree has no `.godot/` global class cache, so the suite hard-fails on `class_name` resolution ("Could not find type ..."). Run `"$GODOT" --headless --import --path .` once before any test run. (godot-ai `filesystem_manage op=reimport` silently no-ops in a brand-new unscanned dir — see `docs/godot-gotchas.md`.)
- **Don't run `tests/run_tests.sh` concurrently in two worktrees.** The runner captures each test's output to a fixed `/tmp/<testname>.out` path shared across checkouts — concurrent suite runs clobber each other's verdicts. Stagger them.
- **godot-mcp stays single-editor.** Its WS bridge (:6550, editor-is-server) is single-client, and a port override would live in the checked-in `project.godot` (shared by all worktrees) — so only ONE editor instance gets godot-mcp's read/test tools. The worktree editor is godot-ai-only; do `godot_input` injection and `godot_runtime_state` verification in the main editor after merge.
...
- `mcp__godot-mcp__godot_editor get_log_messages` (`godot_editor_read` on 4.1.0; **no `source` arg on either version — it has never existed and is silently stripped**; on 4.1.0 use `severity` / `since` instead. For true editor-only logs use godot-ai `logs_read source="editor"`) — editor-side log with `file` + `line` for parser errors, script load failures, library conflicts, plus engine errors. **Primary tool when the scene won't start.** (`get_errors` and `get_debug_output` were removed in v3.6.1 — this replaces them.)
...
**`mcp__godot__get_diagnostics` is not authoritative.** It catches LSP-visible errors in a single file but misses engine-side parse failures that involve cross-script symbol resolution. Example: `_player.is_steering()` where `_player` is typed `CharacterBody3D` and `player.gd` has no `class_name` — `is_steering()` resolves to Variant at engine parse time, breaks `:=` inference, fails to load the script. `get_diagnostics` reports the file clean. **After writing GDScript that touches another script's exports / methods / signals, cross-check godot-ai `logs_read source="editor"` (or godot-mcp `get_log_messages severity="error"` on 4.1.0) before declaring success.**

Proactively reach for godot-ai `logs_read source="editor"` when the user reports failure — don't wait to be asked.
...
**Raw physical-keycode handlers → godot-ai `game_manage input_key` (NOT `godot_input`).** `godot_input` drives only Input-Map *actions*, so an `_unhandled_input` handler matching a raw `event.physical_keycode == KEY_*` — e.g. unmapped debug keys that deliberately dodge the input map (`godot-personal-gotchas` #46) — is invisible to it. godot-ai `game_manage op=input_key {key:"F10", pressed:true}` injects a real key event carrying the **physical** keycode, so it drives those handlers directly; pair it with `editor_manage op=game_eval {code:"..."}` to read the resulting live state (e.g. a node's debug-readout method) with no screenshot round-trip. Real-keyboard caveat: on macOS top-row F-keys send media events unless **Fn** is held (or "use F1/F2… as standard function keys" is on) — the `physical_keycode` binding is correct, the delivery isn't (`godot-personal-gotchas` #54).

**Two gotchas on `godot_input`:**
- **The discriminator key is `action`, not `operation`** — one of `get_map` | `sequence` | `type_text`. Passing `operation=...` (or any other discriminator value, e.g. `mouse_move`/`get_mouse`) fails with `Invalid discriminator value`.
- **No mouse / cursor-coordinate support in `godot_input`.** It cannot move the pointer or click a screen position. godot-ai's `game_manage input_mouse` *does* inject raw mouse button/motion events (for handlers reading `event.position`), **but** a handler reading `get_global_mouse_position()` still won't see them — that reads the OS cursor, not the injected event (`docs/godot-gotchas.md` #32). So for cursor-**aim** mechanics (an aim indicator, a node reading `get_global_mouse_position()`) there's still no clean injection route: move the real OS mouse and verify with a `source="game"` screenshot, hold the action stationary (see next bullet), or temporarily drive the value from a script. (`godot_runtime_state` is likewise `action`-discriminated: `digest` | `watch_start` | `watch_collect` | `watch_stop` — not `operation`/`get_property`.)

**Verifying a cursor-*aimed* mechanic anyway.** If a mechanic aims only *indirectly* through a cursor-eased transform (e.g. a tool/weapon fired along an actor's facing, where the facing springs toward the cursor) rather than reading `get_global_mouse_position()` directly, you CAN verify it headlessly without cursor control: inject the named activation action (`godot_input` `sequence`, long `duration_ms` = held) while the actor sits **stationary among the targets**, then read the effect numerically with `godot_runtime_state digest` (e.g. a group-count delta) and/or a `source="game"` screenshot of the before/after. Three traps:

- **Don't also inject a strong *movement* action to "drive through" the targets.** The actor can cross the catch zone faster than the mechanic acts on it (a wide catch radius traversed in a fraction of a second vs a slower per-frame reel/pull), so nothing is caught and the count stays flat — a false negative that looks like a broken mechanic. Keep the actor stationary, or drift *gently*.
- **The injection holds the editor main thread for `duration_ms`,** so a `source="game"` screenshot issued in the *same tool batch* lands **after** the hold (the action is already released, nothing transient drawn). You cannot capture the during-hold visual that way — accept the after-state screenshot + the numeric `digest` delta as proof of behavior, and leave the live transient visual to a human F5.
- **`godot_runtime_state digest`'s `entity_count` is the *returned* node count, capped by `max_nodes` (default 40)** — not the true group size. Pass `max_nodes` ≥ the expected group size (with `select="group"`, `group="<name>"`) or a truncated read looks like shrinkage. The true count is `entity_count` only when it's strictly below `max_nodes`.
