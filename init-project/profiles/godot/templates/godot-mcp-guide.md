...
- `mcp__godot-mcp__godot_editor get_log_messages` (**no `source` arg — it has never existed and is silently stripped**; on 4.1.0 use `severity` / `since` instead. For true editor-only logs use godot-ai `logs_read source="editor"`) — editor-side log with `file` + `line` for parser errors, script load failures, library conflicts, plus engine errors. **Primary tool when the scene won't start.** (`get_errors` and `get_debug_output` were removed in v3.6.1 — this replaces them.)
