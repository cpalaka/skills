...
- **No mouse / cursor-coordinate support.** It cannot move the pointer or click a screen position — there is no injection path for cursor input. To exercise cursor-relative behavior (a node reading `get_global_mouse_position()`, an aim indicator, etc.), there's no headless route: move the real OS mouse and verify with a `source="game"` screenshot, or temporarily drive the value from a script. (`godot_runtime_state` is likewise `action`-discriminated: `digest` | `watch_start` | `watch_collect` | `watch_stop` — not `operation`/`get_property`.)
